...
import { default as meta } from "./package.json" with { type: 'json' };
...
const allTools = (await import(path.resolve(__dirname, "vizabi-tools.json"),{
  with: {
    type: "json",
  }
})).default;
const toolset = (await import(path.resolve(__dirname, "src", "config", `toolset.${__PROD__ ? (__STAGE__ || "prod") : "dev"}.json`), {
  with: {
    type: "json",
  }
})).default;
...
//toolspage  
  ...
  //perf: true,
    ...
    //__PROD__ &&
      ...
      legacy({
        [require.resolve("@vizabi/reader-ddfcsv/dist/reader-ddfcsv.js")] : "DDFCsvReader"
      }),
      copy({
        targets: [
          ...
          { src: [ "@vizabi/shared-components/build/VizabiSharedComponents.css", ...getEntryToolsCssFilenames()].map(css=>require.resolve(css)), dest: "build/tools/assets/css" }
        ],
        copyOnce: true,
        verbose: false
      }),
