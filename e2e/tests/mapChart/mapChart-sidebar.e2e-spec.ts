...
describe('Maps chart: Sidebar', () => {
  ...
  it('Countries could be selected/deselected using the search in sidebar', async () => {
    ...
    expect(await mapChart.countSelectedCountries.count()).toEqual(1);

    await sidebar.findSelect.searchAndSelectCountry('India');
    expect(await mapChart.countSelectedCountries.count()).toEqual(2);

    expect(await mapChart.countSelectedCountries.getText()).toMatch('China');
    expect(await mapChart.countSelectedCountries.getText()).toMatch('India');
    ...
    expect(await mapChart.countSelectedCountries.count()).toEqual(0);
